...
class LVISPerImageEval:
    ...
    def accumulate(self):
        ...
        precision = -np.ones(
            (num_thrs, num_recalls, num_imgs, num_area_rngs)
        )
        recall = -np.ones((num_thrs, num_imgs, num_area_rngs))
        ...
        for img_idx in range(num_imgs):
            ...
            for area_idx in range(num_area_rngs):
                dt_pointers[img_idx][area_idx] = {}

        # Per image evaluation
        for img_idx in range(num_imgs):
            Na = img_idx * num_area_rngs * num_cats
            for area_idx in range(num_area_rngs):
                Nk = area_idx * num_cats
                E = [
                    self.eval_imgs[Nk + Na + cat_idx]
                ...
                for iou_thr_idx, (tp, fp) in enumerate(zip(tp_sum, fp_sum)):
                    ...
                    rc = tp / num_gt
                    if num_tp:
                        recall[iou_thr_idx, img_idx, area_idx] = rc[
                            -1
                        ]
                    else:
                        recall[iou_thr_idx, img_idx, area_idx] = 0
                    ...
                    try:
                        ...
                        pass
                    precision[iou_thr_idx, :, img_idx, area_idx] = np.array(pr_at_recall)
