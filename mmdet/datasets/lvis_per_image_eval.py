...
from .lvis_per_image_eval_class import LVISPerImageEval
...
class LVISV05Dataset(CocoDataset):
    ...
    def evaluate(self,
        ...
        for metric in metrics:
            ...
            lvis_eval = LVISPerImageEval(lvis_gt, lvis_dt, iou_type)
            ...
            if metric == 'proposal':
                ...
                lvis_eval.accumulate()
                ###############
#                 print(len(lvis_eval.eval_imgs))         

                
#                 lvis_eval.summarize()
                lvis_results = lvis_eval.get_results()
                if imagewise:  # Compute per-category AP
                    # Compute per-image AP
                    # from https://github.com/facebookresearch/detectron2/
                    precisions = lvis_eval.eval['precision']
                    # precision: (iou, recall, cls, area range, max dets)
                    assert len(self.img_ids) == precisions.shape[2]

                    results_per_image = []
                    for idx, imgId in enumerate(self.img_ids):
                        # area range index 0: all area ranges
                        # max dets index -1: typically 100 per image
                        nm = self.coco.load_imgs([imgId])[0]
...
#                         precision = precisions[:, :, idx, 0, -1]
                    ...
                    num_columns = min(6, len(results_per_image) * 2)
                    results_flatten = list(
                        itertools.chain(*results_per_image))
                    headers = ['Image Id', 'AP'] * (num_columns // 2)
                    ...
                    # save classwise AP /// changed by satyam
                    np.save("imagewise_AP.npy", np.array(table_data))
